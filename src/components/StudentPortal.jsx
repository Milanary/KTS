...
export default function StudentPortal({ onLogout, theme, toggleTheme }) {
  ...
  const [submittedFileName, setSubmittedFileName] = useState(() => {
    return localStorage.getItem('kts_submitted_file_name') || 'Assignment_Week2_Submission.pdf';
  });
  const [submittedAt, setSubmittedAt] = useState(() => {
    return localStorage.getItem('kts_submitted_at') || 'June 28, 2026 at 02:15 PM';
  });
  const [assignmentScore, setAssignmentScore] = useState(() => {
    ...
    if (saved === 'null' || saved === null || saved === '5.5') {
  ...
  const [assignmentComment, setAssignmentComment] = useState(() => {
    ...
    if (saved === 'null' || saved === null || saved.includes("reliance on automated")) {
      localStorage.setItem('kts_assignment_comment', defaultComment);
      return defaultComment;
    }
    return saved;
  });
  const [assignmentSec2Score, setAssignmentSec2Score] = useState(() => {
    const saved = localStorage.getItem('kts_assignment_sec2_score');
    const defaultScore = '5.5';
    if (saved === 'null' || saved === null) {
      localStorage.setItem('kts_assignment_sec2_score', defaultScore);
      return defaultScore;
    }
    return saved;
  });
  const [assignmentSec2Comment, setAssignmentSec2Comment] = useState(() => {
    const saved = localStorage.getItem('kts_assignment_sec2_comment');
    const defaultComment = "The submitted analysis for the practical case studies clearly indicates an extensive reliance on AI-generated content rather than genuine human effort and personal reasoning. Please do not repeat this approach in future submissions, as continued use of automated text will not be tolerated.";
    if (saved === 'null' || saved === null) {
      localStorage.setItem('kts_assignment_sec2_comment', defaultComment);
      return defaultComment;
    }
    return saved;
  });
        ...
        <main className="dashboard-content">
          ...
          {activeMenu === 'assignments' && (
            <section className="assignment-section" style={{ animation: 'fadeIn 0.4s ease' }}>
              ...
              <div className="assignment-card" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
                  <div>
                    ...
                    <h2 style={{ fontSize: '24px', fontWeight: '800', margin: '8px 0 4px', color: 'var(--text-primary)' }}>
                      Assignment Week2: Balancing Duty of Care with Dignity of Risk
                    </h2>
                    <p style={{ fontSize: '14px', color: 'var(--text-secondary)', margin: 0 }}>
                      Competency Assessment: Clinical Care and Ageing Support.
                    </p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <span className="assignment-badge completed" style={{ background: 'rgba(245, 158, 11, 0.1)', color: '#d97706', border: '1px solid rgba(245, 158, 11, 0.2)' }}>
              ...
              {/* Two Sections of Marking */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                {/* Section 1 Card */}
                <div className="assignment-card" style={{ margin: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', marginBottom: '16px' }}>
                      <div style={{
                        ...
                        <span style={{ fontSize: '24px', fontWeight: '900', color: 'var(--text-primary)', margin: '2px 0' }}>{assignmentScore}</span>
                  ...
                  <div style={{ borderTop: '1px dashed var(--border-card)', paddingTop: '12px', marginTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '11px', color: 'var(--success)', fontWeight: '800', background: 'rgba(16,185,129,0.08)', padding: '2px 8px', borderRadius: '4px', border: '1px solid rgba(16,185,129,0.15)' }}>Passed</span>
                  </div>
                </div>
                ...
                <div className="assignment-card" style={{ margin: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', marginBottom: '16px' }}>
                      <div style={{
                        ...
                        <span style={{ fontSize: '10px', fontWeight: '800', color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Grade</span>
                        <span style={{ fontSize: '24px', fontWeight: '900', color: 'var(--text-primary)', margin: '2px 0' }}>{assignmentSec2Score}</span>
                        <span style={{ fontSize: '10px', color: 'var(--text-secondary)' }}>out of 10.0</span>
                      </div>
                  ...
                  <div style={{ borderTop: '1px dashed var(--border-card)', paddingTop: '12px', marginTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Evaluated by: <strong>Sheren Molt</strong></span>
                    <span style={{ fontSize: '11px', color: '#d97706', fontWeight: '700', background: 'rgba(245,158,11,0.08)', padding: '2px 8px', borderRadius: '4px' }}>Passed with Warning</span>
                  </div>
                </div>
              </div>
            </section>
