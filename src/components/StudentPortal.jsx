...
const getCourseLessons = (course) => {
  if (!course) return [];
  return course.lessons || Array.from({ length: course.lessonsCount }, (_, i) => i + 1);
};
...
const COURSE_LESSON_CONTENT = {
  ...
  'chcleg003': {
    ...
    '2.1': {
      title: 'Rights-Based Aged Care Act and Regulatory Framework',
      desc: 'Explore the transition to a rights-based aged care system, understanding regulatory frameworks, compliance actions, and human rights approaches in Australian care services.',
...
const DEFAULT_ASSIGNMENTS = [
  {
    id: 'week1',
    title: 'Assignment Week1: Facilitate the empowerment of older people',
    dueDate: 'June 18, 2026 at 11:59 PM',
    status: 'Graded',
  ...
  {
    id: 'week2',
    title: 'Assignment Week2: Balancing Duty of Care with Dignity of Risk',
    dueDate: 'June 25, 2026 at 11:59 PM',
    status: 'Graded',
...
export default function StudentPortal({ onLogout, theme, toggleTheme }) {
  ...
  const [submissions, setSubmissions] = useState(() => {
    ...
    localStorage.setItem('kts_assignments_submissions', JSON.stringify(defaultSubmissions));
  ...
  const handleAssignmentSubmit = (assignmentId) => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('kts_assignments_submissions', JSON.stringify(nextSubmissions));
  ...
  const handleResetAssignmentSubmission = (assignmentId) => {
  ...
  const getCourseCompletedLessons = (courseId) => {
    ...
    const lessonsList = getCourseLessons(course);
    lessonsList.forEach((lessonKey) => {
      if (progress[`${courseId}-${lessonKey}`] === true) {
        completed++;
      }
    });
    return completed;
  ...
  const getActiveLessonContent = () => {
    ...
    return {
      title: `Lesson Module ${currentLessonIndex}: Competency Outline`,
  ...
  const mockSchedule = [
    {
      ...
      hour: '09:30',
      ampm: 'AM',
      courseCode: 'CHCCCS011',
      subject: 'Clinical Competency & Field Operations Guide Class',
      ...
      <div className="main-canvas">
        ...
        <main className="dashboard-content">
                          ...
                          <div className="course-footer" style={{ borderTop: 'none', paddingUp: 0, marginTop: 12 }}>
                            ...
                            <button 
                              ...
                              onClick={() => {
                                ...
                                setCurrentLessonIndex(getCourseLessons(course)[0]);
                ...
                <section className="classroom-container">
                  ...
                  <div className="classroom-sidebar">
                    ...
                    <div className="lessons-list">
                      {getCourseLessons(selectedCourse).map((lessonKey) => {
                        const key = `${selectedCourse.id}-${lessonKey}`;
                        const isCompleted = progress[key] === true;
                        const lessonDetails = COURSE_LESSON_CONTENT[selectedCourse.id]?.[lessonKey] || { title: `Lesson Module ${lessonKey}` };
                        return (
                          <div 
                            key={lessonKey} 
                            className={`lesson-item ${currentLessonIndex === lessonKey ? 'active' : ''}`}
                            onClick={() => {
                              setCurrentLessonIndex(lessonKey);
                              setIsPlayingClassroomVideo(false);
                            }}
                          >
                            <div className="lesson-info">
                              <span className="lesson-num">{lessonKey}</span>
                      ...
                      <div className="classroom-header">
                        <div>
                          ...
                          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>
                            Unit Topic {getCourseLessons(selectedCourse).indexOf(currentLessonIndex) + 1} of {selectedCourse.lessonsCount}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="classroom-action-row">
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        <button
                          type="button"
                          className="classroom-back-btn"
                          disabled={getCourseLessons(selectedCourse).indexOf(currentLessonIndex) === 0}
                          onClick={() => {
                            const lessons = getCourseLessons(selectedCourse);
                            const idx = lessons.indexOf(currentLessonIndex);
                            if (idx > 0) {
                              setCurrentLessonIndex(lessons[idx - 1]);
                              setIsPlayingClassroomVideo(false);
                            }
                          }}
                        >
                        ...
                        <button
                          ...
                          disabled={getCourseLessons(selectedCourse).indexOf(currentLessonIndex) === selectedCourse.lessonsCount - 1}
                          onClick={() => {
                            const lessons = getCourseLessons(selectedCourse);
                            const idx = lessons.indexOf(currentLessonIndex);
              ...
              <div className="section-card" style={{ maxWidth: '600px', margin: '40px auto', padding: '32px', textAlign: 'center', borderRadius: '16px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', backgroundColor: '#fff', border: '1px solid var(--border-card)' }}>
                ...
                <div style={{ margin: '24px 0' }}>
                  ...
                  <h2 style={{ fontSize: '24px', fontWeight: '700', color: 'var(--text-primary)', marginTop: '16px', marginBottom: '8px' }}>
                    Clinical Competency & Field Operations Guide Class
                  </h2>
                  <p style={{ fontSize: '14px', color: 'var(--text-secondary)', margin: '0 auto 32px', maxWidth: '400px' }}>
                    CERT IV in Ageing Support (CHCCCS011). The live session with Milan Aryal will begin shortly.
                  </p>
                  ...
                  <div style={{ 
                    ...
                    {timeLeft > 0 ? (
                      <>
                        ...
                        Waiting for class to unlock at 09:30 AM Nepal Time...
                      </>
                    ) : (
                      <>
          ...
          {activeMenu === 'assignments' && (() => {
            const activeAssignment = DEFAULT_ASSIGNMENTS.find(a => a.id === selectedAssignmentId) || DEFAULT_ASSIGNMENTS[0];
            const activeSubmission = submissions[activeAssignment.id] || { submitted: false, fileName: '', submittedAt: '' };
            ...
            <div className="pdf-modal-card" onClick={(e) => e.stopPropagation()}>
              <div className="pdf-modal-header">
                <h3 className="pdf-modal-title">
                  ...
                  <span>Preview: {activeAssignment.fileName}</span>
                </h3>
                <button
                  type="button"
              ...
              <div className="pdf-modal-body">
                <iframe
                  src={`${activeAssignment.downloadUrl}#toolbar=0`}
                  title={`${activeAssignment.title} Preview`}
                  style={{
                    width: '100%',
                    height: '100%',
                    border: 'none',
